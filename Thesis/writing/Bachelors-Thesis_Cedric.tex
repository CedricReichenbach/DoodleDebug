...
Navigation between nesting layers inside a pop-up is aided with bread crumbs~\cite{Krug00a} \nes{Page?} \nes{Well? What page?} \meta{Sorry, I never touched that book. Who made the reference?}, which traces the currently inspected branch of the object graph (\figref{breadcrumbs}).
...
Distinguishing between them enables semantic zoom \cite{semantic-zoom} \meta{Where/when was semantic zoom first mentioned? Is it in the SCG bib?} when inspecting doodles.
...
A \texttt{Contact} class could implement its \texttt{Doodleable} methods as follows:
\begin{lstlisting}
public void doodleOn(DoodleCanvas c) {
	c.draw(firstName);
	c.draw(lastName);
...
\end{lstlisting}
\Figref{canvas-illustration} illustrates how the output resulting from this code is generated.
