...
\nes{The book `Code complete` has a nice section on debugging that may be worth quoting. Also relevant: The practice of programming, http://cm.bell-labs.com/cm/cs/tpop/debugging.html}
...
\nes{In a sense, DoodleDebug is *orthogonal* to the control flow features of debuggers. (Please adapt here and in the paper.)}
...
\nes{I'm missing a pithy summary of what Doodledebug offers. (1) a log of past state, (2) zoom into the log, (3) a decent API for doodling. Should be offered as a list. Here and in the paper.}
...
During our study (\secref{study}), subject Echo manually disabled such notifications of the Eclipse console, complaining about it stealing the focus of the UI part where they were working in.

Users that debug a program that rarely produces output, like caught exceptions, \ugh{they} probably want to be notified about this event and maybe inspect the new doodle. \nes{Needlessly fuzzy. Make stronger.}
...
Objects that are nested into others are not graphically scaled down\ins{,} nor removed, but rendered in its \nes{grammar} summarized, less detailed  version.

In \DD, objects rendered to nesting levels 0 \ugh{(doodled ones)} and 1 \nes{Level numbering not explained.} are rendered with full detail; objects of nesting level 2 (inside level 1) show their summarized version.
...
Navigation between nesting layers inside a pop-up is aided with bread crumbs~\cite{Krug00a} \nes{Page?} \nes{Well? What page?}, which traces the currently inspected branch of the object graph (\figref{breadcrumbs}).
...
A developer observing this output \ugh{might} be interested in more detail of one particular state, which can be done by clicking on parts of it, resulting in a pop-up (\figref{game_last-state}).
...
\nes{terrible way of phrasing it. When I read stuff like that, I imagine a horribly over-engineered crappy framework, worse than Apache io.
Tone this down a lot. `At the plugin level, the user can directly control the generation of html, css, and Javascript.`}
users may \ugh{also} provide \texttt{RenderingPlugin}s, which are also used internally for \DD's built-in renderings.
\ugh{However}, \texttt{Doodleable} customizations are always preferred over plugins when both are available for a type.
...
\nes{Why discuss in the abstract? Show the code, or reference it. Here and in the paper.}
...
\nes{You discussed plugins above. Don't do it again. Do it in one place only.}
...
The principle of semantic zoom is \ugh{again} retained through two different methods for different detail levels.
...
As output format, we decided \nes{No one cares what we decide.} to use HTML, which enables more powerful\nes{Again, `powerful` is just not the class of word you can use here. What do  you mean?} formatting than plain text with moderate implementation effort. \nes{Probably, delete entire sentence.}
...
As a consequence, we had to be careful when creating HTML output and always test \ugh{them} \nes{grammar} in different browsers to prevent differences in design on different systems.
All major changes or extensions affecting the output were tested in four Browsers with different rendering engines: Gecko (Firefox), Presto (Opera), Webkit (Chrome) \nes{Webkit is actually at home in Safari.} and Trident (Internet Explorer).
On Windows systems, for instance, Eclipse uses the Internet Explorer rendering engine.
Thus, a \texttt{meta} tag needed to be set in order to suppress compatibility \nes{It's called `quirks mode', I think. Please add a reference. Here and in the paper.} modes and make sure the newest installed rendering engine is used for best-possible support, especially for CSS3 features.
...
A fully functional release candidate of DoodleDebug (version 0.9.0 for Alpha and Bravo, 0.9.1 for others) was used.
...
At this time, the instructor was allowed to answer questions and support the subject, both on DoodleDebug and standard tools.
...
The permission to use DoodleDebug on a particular problem changed with every study session, i.e. \nes{Please look up how to write i.e. in latex. The paper might have a template called "\ie".}  if subject 1 was allowed to use DoodleDebug on problem A, then subject 2 would not be allowed, but subject 3 would.
...
\nes{It would be mildly interesting to know how he justified that.}
...
Echo bespoke confusion \nes{Oscar marked this, I believe. Please rephrase.} about the difference between the two methods in the \texttt{Doodleable} interface. After going back to the documentation of the interface, he understood.
...
When certain output is printed onto the console \nes{Eclipse console}, it gains the UI's focus by default. \nes{You're talking about Eclipse things. Please make clear.}
...
\meta{Like that? Looks a bit strange to me... Delete (parts of it)?}
\nes{Yes.}
...
\meta{What if it's in the "Related Work" section?}
\nes{Much better.}
...
\meta{Update: It's a conclusion, not a summary, right? Or should I summarize like in the study summary?}
\nes{Yea, it should be a conclusion.}
...
Getting started is rather easy since the setup work is mainly done by Eclipse and the API is simple; \nes{Don't raise new points in the conclusion. Only pick out things that were said before. Either prepare for "easy setup", or delete.} knowing one single method already enables the core features of \DD.
Continuous migration from classical tools to \DD is not a problem, since parallel usage works flawlessly. \nes{I'm not sure what this means. re-write sentence.}
