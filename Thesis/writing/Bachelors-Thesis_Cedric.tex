...
It analysed \nes{Unless you have strong opinions about this, use the "z" form in words ending on "-ize". It's acceptable in both BE and AE.} SqueakSource, a hosting service for Smalltalk projects, searching for \texttt{printOn} methods, Smalltalk's equivalent to Java's \println.
\todo{Results}
...
After a request for doodling an object has been received, \DD analyses \nes{Use the z.} its type and searches for a fitting rendering in the different customization layers.
...
\nes{The word is "convenience sampling". Don't explain "were not unlimited" -- it's not a proper trade-off discussion. Kill or expand.}
...
Before \ugh{heading on the study problems}, every user had 15-30 minutes to work through a tutorial, play around with \DD in a sandbox and ask questions to the instructor.
When the actual study problems had been revealed, the instructor \ugh{cancelled}\nes{Careful: this is British spelling again. Also: it's the wrong word to use here.} their support.
...
\nes{Both here and in the paper, the study needs some more outlining, and more structure.}
