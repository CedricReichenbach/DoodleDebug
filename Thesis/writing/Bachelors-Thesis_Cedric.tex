...
\item A decent API for doodling objects
\end{enumerate}
...
Every object directly referenced inside this one renders to level 1, those referenced from level 1 render to level 2 and so on.

In \DD, objects rendered to nesting levels 0 and 1 are rendered with full detail; objects of nesting level 2 show their summarized version.
...
Those include:
\nes{Where can I see them?}\todo{Maybe links to figures into paper?}
...
\begin{figure}[h]
	\includegraphics[width=0.4\linewidth]{img/FieldDoodler_player.png}
	\caption{The standard rendering of an object, visualizing all of its fields.}
	\figlabel{fielddoodler-player}
\end{figure}

\begin{figure}[h]
	\includegraphics[width=0.8\linewidth]{img/matrix-array.png}
