...
\meta{Wha-}
\nes{A discussion that Doodledebug is orthogonal.}
\meta{Damn, I forgot to finish the previous sentece... What do you mean by orthagonal? I'm trying to understand a program as a multidimensional room, say n+1 dimensions. Then you would have one dimension for time (i.e. program flow) and at each point in time (where you stop the debugger), there's an n-dimensional slice of program content. DoodleDebug can keep those slices alive whereas a debugger only shows one at a time. But how is this orthagonal?}
\nes{By orthogonal I mean that they are independent. That is, the user experience of flow control in a debugger does not depend on the user experience of logging and showing state, and vice versa. Thus, our code can be plugged into a debugger, and a debugger can be plugged into DD.}
...
Those include:
\todo{Maybe links to figures into paper?}
...
\nes{What you're saying is: `receive a DoodleCanvas' object.' That's too abstract a level of discussion. Show the code.}
...
\meta{(added comment from pdf:)}
\on{Well, I think it is fair to say that the tasks have been designed specifically to see whether DD is indeed better than println for certain kinds of problems. A very different question is whether these problems are representative of usual programming tasks.}
