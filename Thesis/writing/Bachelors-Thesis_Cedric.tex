...
As McConnell states in Code Complete\cite[p. 539]{code complete}\meta{I couldn't find "Code Complete" in the SCG bib file, but I'm pretty sure it must be there...}, debuggers help to avoid bad practices caused by debugging with \println, like scattering print statements randomly throughout a program.
...
\meta{The following sentence might be a bit bumpy...}
...
\meta{(original comment from pdf:)}
...
Debugging by directly writing into source code may be easier and quicker than having to equip other debugging tools for a session\cite{some guy}\todo{WHO?}.
