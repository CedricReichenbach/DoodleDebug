...
\subsection{Default renderings}
We knew from previous work that 77 \% of \texttt{toString()} methods print out the class name \cite{Schw11b}. We therefore tried to find renderings that always show the class name, but do so in a space-efficient way. \nes{Cedric: this needs pictures …}

\nes{Can I get screenshots of all rendering plugins?!}

\subsection{API design process}
As soon as we had a consistent imaginary API and enough sample sketches, we stuck them to a wall and asked colleagues and other passing by programmers for feedback, as was captured in \Figref{sketch-discussion}. To illustrate the design process, we show some sketches and the feedback that they triggered.
...
\begin{figure}[h]
	...
	\label{fig:sketch-discussion}
\end{figure}

Though most asked programmers managed to quickly understand the sketch in \Figref{bad-sketch_align-h-v}, they immediately asked about further 2D structuring. 
...
Another recognized the similarity with Swing's flow layout manager and found it overly complex, because it requires endless nesting of layouts.
...
\begin{figure}[h]
	...
	\label{fig:bad-sketch_align-h-v}
\end{figure}

In \Figref{bad-sketch_left-right}, we tried a design that would allow to draw objects either on the left or the right part of its line, or just to take the line's full width. 
...
However, people misunderstood \texttt{drawLeft(object)} to take the left side of a bigger instance, e.g. the whole canvas. \nes{Eh? Don't understand.}}
	\label{fig:bad-sketch_left-right}
\end{figure}

Since UML specifies object instance visualizations\footnote{\url{http://www.omg.org/spec/UML/2.4.1/}} \nes{Better way to cite UML?}, we considered using or at least supporting UML doodles that are valid UML drawings. A sketch is seen in \Figref{bad-sketch_uml}.
No one of the asked programmers supported this idea. 
None had ever used UML object visualizations (although they had used UML class visualizations), and were unconvinced of the aesthetic value of the UML-prescribed layout. 

\begin{figure}[h]
	\includegraphics[width=\linewidth]{img/sketches/006.jpg}
	\caption[Bad sketch example: UML]{Code that produces valid UML object visualizations.}
	\label{fig:bad-sketch_uml}
\end{figure}

In a sketch that came close to the final DoodleDebug, objects from one column could trespass over into other columns to use the screen estate efficiently, see \Figref{good-sketch_cursor}. While people understood the cursor concept readily and were enthusiastic about it, the non-strictness of columns confused them. 
As a consequence, the final Canvas API adopted the traditional textbook pattern and strictly separates columns, which made the separator method obsolete as well.

\begin{figure}
	\includegraphics[width=\linewidth]{img/sketches/003.jpg}
	\caption[Good sketch: Cursor paradigm]{An successor of the final Canvas API.}
	\label{fig:good-sketch_cursor}
\end{figure}
...
The DoodleDebug Eclipse plugin, all source code a demo video are available on \url{http://scg.unibe.ch/wiki/projects/DoodleDebug}. \nes{Cedric: please make it so :)}
