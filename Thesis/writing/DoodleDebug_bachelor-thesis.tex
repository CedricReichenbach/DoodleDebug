...
For spacial navigation on a virtual canvas, a mechanism working similar to a cursor of a simple text editor turned out to be most intuitive for humans. \nes{"turned out" is not good enough. Show the sketches that didn't work, and the gradual evolution to the thing that did work.}
...
We attribute the supremacy of this method to the principle of standardization\cite[pp. 200--203]{Norm88a}. \nes{What is standardization? Explain.} Virtually traversing an area from top-left to bottom-right, separated in lines and eventually in columns, is the way how text is written in western languages. Especially programmers, who use text editors on a daily basis, can therefore quickly adapt and apply this principle. \nes{Well ... another argument is that this is system.out.println uses a cursor position, and developers understand system.out.println. Please discuss this in the related work section.}
...
\nes{No "previously stated". No "etc".}
...
CodeBubbles's\cite{Brag10a} debugger allows to open bubbles by hand that represent states in the debugger. Thus, many old states can be on-screen. However, all old state is triggered thru direct user interaction. Unlike System.out.println, or DoodleDebug, where all printing happens from the program.

\todo{What further related work is worth mentioning?}
\nes{This gaming guy that would overlay time frames in his output.}
...
\section{Future Work}
\nes{This is out of hand. We may or may not keep the "split doodleable interface" thing. Though I still think that should just be fixed and listed as an accomplishment, not as future work. The rest of this section should go.}
